...
%        result.pval   = pvalue of LR-ratio test against intercept model
...
while (iter < maxit) & (crit > tol)
    ...
    bn = b + s*db;
    crit = abs(max(max(db)));
    if  (abs(sum(y-delta)) < tol && max(tmp_inf) == Inf) || (max(tmp_inf) == Inf)
        perfit_flag = 1;
    end
...
result.pval = 1-chi2cdf(result.lratio,k-1); % p-value of LR-ratio test against intercept model
...
result.AIC = -like1+1/2*k; % Akaike IC
result.BIC = -like1+1/2*k*log(t); % BIC IC
